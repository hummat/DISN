...
def create_sdf(sdfcommand, marching_cube_command, LIB_command, num_sample,
    ...
    for catnm in cats.keys():
        ...
        if not os.path.exists(raw_dirs["norm_mesh_dir"]):
            os.makedirs(raw_dirs["norm_mesh_dir"])
        cat_mesh_dir = os.path.join(raw_dirs["mesh_dir"], cat_id)
        cat_norm_mesh_dir = os.path.join(raw_dirs["norm_mesh_dir"], cat_id)
        with open(lst_dir + "/" + str(cat_id) + "/test.lst", "r") as f:
            list_obj = f.readlines()
        with open(lst_dir + "/" + str(cat_id) + "/train.lst", "r") as f:
            list_obj += f.readlines()
        with open(lst_dir + "/" + str(cat_id) + "/val.lst", "r") as f:
            list_obj += f.readlines()
        # print(list_obj)
        repeat = len(list_obj)
...
if __name__ == "__main__":
    ...
    create_sdf(sdfcommand="./isosurface/computeDistanceField",
               marching_cube_command="./isosurface/computeMarchingCubes",
               LIB_command="bash source isosurface/LIB_PATH",
               num_sample=32768,  # points sampled from the res^3 available points
               bandwidth=0.1,  # max distance from surface during sampling
               res=256,  # sdf value grid resolution
               expand_rate=1.2,
               cats=cats,
               raw_dirs=raw_dirs,
               lst_dir=lst_dir,
    ...
    create_sdf("./isosurface/computeDistanceField",
               ...
               "bash source isosurface/LIB_PATH", 32768, 0.1,
