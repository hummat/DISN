...
def create_h5_sdf_pt(cat_id, h5_file, sdf_file, flag_file, norm_obj_file,
    ...
    samplesdf, is_insideout = sample_sdf(cat_id, num_sample, bandwidth, iso_val, sdf_dict, sdf_res,
                                         reduce)  # (N*8)x4 (x,y,z)
    if is_insideout:
...
def create_sdf_obj(sdfcommand, marching_cube_command, cat_mesh_dir, cat_norm_sdf_dir, cat_sdf_dir, obj,
                   res, iso_val, expand_rate, indx, ish5, normalize, num_sample, bandwidth, max_verts, cat_id, g,
                   reduce):
    obj = obj.rstrip('\r\n')
