...
def get_model(ref_dict, num_point, is_training, bn=False, bn_decay=None, img_size=224, wd=1e-5, FLAGS=None):
    ...
    with slim.arg_scope([slim.conv2d], weights_regularizer=slim.l2_regularizer(wd)):
        ref_feats_embedding, vgg_end_points = vgg.vgg_16(ref_img, num_classes=FLAGS.num_classes, is_training=False,
                                                         scope='vgg_16', spatial_squeeze=False)
        ...
        if FLAGS.threedcnn:
            ...
            if not FLAGS.multi_view:
                with tf.variable_scope("sdfimgfeat") as scope:
                    vgg_conv1 = tf.image.resize_bilinear(vgg_end_points['vgg_16/conv1/conv1_2'],
                                                         (FLAGS.img_h, FLAGS.img_w))
                    point_vgg_conv1 = tf.contrib.resampler.resampler(vgg_conv1, sample_img_points)
                    vgg_conv2 = tf.image.resize_bilinear(vgg_end_points['vgg_16/conv2/conv2_2'],
                                                         (FLAGS.img_h, FLAGS.img_w))
                    point_vgg_conv2 = tf.contrib.resampler.resampler(vgg_conv2, sample_img_points)
                    vgg_conv3 = tf.image.resize_bilinear(vgg_end_points['vgg_16/conv3/conv3_3'],
                                                         (FLAGS.img_h, FLAGS.img_w))
                    ...
                    pred_sdf = sdfnet.get_sdf_basic2_imgfeat_onestream_binary(ref_sample_pc_rot,
                                                                              ref_feats_embedding_cnn,
                                                                              point_img_feat, is_training, batch_size,
                                                                              num_point,
                                                                              bn, bn_decay, wd=wd)
            ...
            if not FLAGS.multi_view:
                ...
                with tf.variable_scope("sdfprediction_imgfeat") as scope:
                    pred_sdf_value_local = sdfnet.get_sdf_basic2_imgfeat_twostream_binary(ref_sample_pc_rot,
                                                                                          point_img_feat,
...
def get_loss(end_points, sdf_weight=10., regularization=True, mask_weight=4.,
    ...
    if FLAGS.binary:
        ...
        accuracy = tf.reduce_mean(
            tf.cast(tf.equal(label_sdf, tf.argmax(pred_sdf, axis=2, output_type=tf.int32)), dtype=tf.float32))
