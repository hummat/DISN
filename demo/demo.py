...
def cam_evl(img_arr):
    with tf.Graph().as_default():
        with tf.device('/gpu:0'):
            ...
            end_points = model_cam.get_model(input_pls, NUM_POINTS, is_training_pl, img_size=(IMG_SIZE, IMG_SIZE),
                                             bn=False, wd=2e-3, FLAGS=FLAGS)
...
def read_img_get_transmat():
    ...
    batch_data['sdf_params'] = np.array(
        [[-1, -1, -1, 1, 1, 1]])  # only useful if we want to compare it with gt, here random set some value
    return batch_data
