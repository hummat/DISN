...
parser.add_argument('--restore_modelpn', default='',
                    help='restore_model')  # checkpoint/sdf_3dencoder_sdfbasic2/latest.ckpt
parser.add_argument('--restore_modelcnn', default='',
                    help='restore_model')  # ../../models/CNN/pretrained_model/vgg_16.ckpt
    ...
    for batch_idx in range(num_batches):
        ...
        if (batch_idx + 1) % verbose_freq == 0:
            ...
            if (batch_idx + 1) % (200 * verbose_freq) == 0:
                ...
                np.savetxt(os.path.join(RESULT_PATH, '%d_sdf_pred.txt' % batch_idx), np.concatenate(
                    (batch_data['sdf_pt_rot'][bid, :, :], np.expand_dims(pred_sdf_val[bid, :, 0], 1)), axis=1))
                np.savetxt(os.path.join(RESULT_PATH, '%d_sdf_ref.txt' % batch_idx), np.concatenate(
                    (batch_data['sdf_pt_rot'][bid, :, :], np.expand_dims(ref_sdf_val[bid, :, 0], 1)), axis=1))
                ...
                output_utils.output_scale_point_cloud(batch_data['sdf_pt_rot'][bid, :, :], pred_sdf_display,
                                                      os.path.join(RESULT_PATH, '%d_pred.obj' % batch_idx))
                output_utils.output_scale_point_cloud(batch_data['sdf_pt_rot'][bid, :, :], ref_sdf_display,
                                                      os.path.join(RESULT_PATH, '%d_gt.obj' % batch_idx))
